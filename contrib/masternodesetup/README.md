...
# QuestionIV Masternode Setup Guide
***
## Required
1) **QIV collateral value at current block** ([consult the collateral table](../../README.md#rewards-breakdown))
...
Remember to do `QuestionIV-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `QuestionIV-cli addnode "ip" onetry`

      Example:
		  QuestionIV-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `QuestionIV-cli getblockcount`, and if yes give it time now to catch last block number 
...
```
QuestionIV-cli getmasternodestatus
```

You need to get **"status" : 4** 

## Congratulations your QuestionIV node it's running
