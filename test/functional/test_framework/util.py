...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s QIV too low! (Should be %s QIV)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 20) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s QIV too high! (Should be %s QIV)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "QuestionIV.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "QuestionIV.conf")):
        with open(os.path.join(datadir, "QuestionIV.conf"), 'r', encoding='utf8') as f:
            for line in f:
