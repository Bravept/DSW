...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter QIV address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no QIV: URI
...
{
    // Convert QuestionIV:// to QuestionIV:
    //
    //    Cannot handle this later, because QuestionIV:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "QuestionIV.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for QuestionIV.lnk
...
{
    return GetAutostartDir() / "QuestionIV.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a QuestionIV.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=QuestionIV\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the QuestionIV app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add QuestionIV app to startup item list
