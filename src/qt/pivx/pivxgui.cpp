...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("QuestionIV client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("QuestionIV"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("QuestionIV"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
