...
#if defined(NDEBUG)
#error "QuestionIV cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uQIV) are considered zero fee (for relaying, mining and transaction creation)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // QuestionIV: We use certain sporks during IBD, so check to see if they are
