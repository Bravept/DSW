...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all QIV specific functionality (Masternodes) (0-1, default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that QuestionIV is running in a usable environment with all
...
#endif
    LogPrintf("QuestionIV version %s (%s)\n", version_string, CLIENT_DATE);
}
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. QuestionIV is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single QuestionIV process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. QuestionIV is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //QuestionIV specific: spork DB's
                ...
                // QuestionIV: load previous sessions sporks if we have them.
